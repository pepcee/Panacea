  ...
  if($password !== $confirmpassword){
    ...
    if($num_rows != 0) {
      ...
      if($stmt->execute()){
        ...
        header('location: account.php?signup=Sign up successful'); 
      }else{
...
<head>
  ...
  <title>Sign Up | Panacea Pharmaceutical</title>
...
<body class="text-gray-800">
  ...
  <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
    <h2 class="text-3xl font-playfair mb-6 text-center">Sign Up</h2>
    <form method="POST" action="signup.php" class="space-y-5">
      ...
      <div>
        <label for="confirm-password" class="block mb-1 font-semibold text-gray-700">Confirm Password</label>
        <input type="password" id="confirm-password" name="confirm-password" placeholder="Confirm Password" required
               class="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-blue-600" />
      </div>

      <button type="submit" name="signup" class="w-full text-white font-semibold py-3 rounded bg-customBlue hover:bg-blue-700 transition">
        Sign Up
      </button>
    </form>
