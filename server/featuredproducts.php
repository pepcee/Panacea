...
if (isset($_GET['id'])) {
  $product_id = $_GET['id'];
  $stmt = $conn->prepare("SELECT * FROM featured WHERE featured_id = ?");
...
<head>
  ...
  <title><?php echo $product['featured_name']; ?> | Panacea</title>
...
<body>
  ...
  <div class="product-container">
    <div class="product-image shadow-xl rounded-lg p-4 bg-white">
      <img src="../<?php echo $product['featured_image']; ?>" alt="Featured">
    </div>
    <div class="product-info">
      <h1><?php echo $product['featured_name']; ?></h1>
      <p class="subtitle"><?php echo $product['featured_tags']; ?></p>
      <p class="price">₱<?php echo $product['featured_price']; ?></p>

      <form method="POST" action="../addtocart.php">
        <input type="hidden" name="featured_id" value="<?php echo $product['featured_id']; ?>"/>
        <input type="hidden" name="featured_image" value="<?php echo $product['featured_image']; ?>"/>
        <input type="hidden" name="featured_name" value="<?php echo $product['featured_name']; ?>"/>
        <input type="hidden" name="featured_price" value="<?php echo $product['featured_price']; ?>"/>
        
        <label for="quantity">Quantity</label>
        <input type="number" id="quantity" value="1" name="featured_quantity" min="1" class="border border-gray-300 rounded px-2 py-1">
        
        <button class="add-tocart" type="submit" name="addtocart">Add to Cart</button> 
      </form>

      <button class="add-btn">Buy Now</button>
      <a href="../index.php">
        <button type="button" class="add-btn">Browse</button>
      </a>

      <div class="description">
        <h2>Product Description</h2>
        <p><?php echo $product['featured_desc']; ?></p>
      </div>
    </div>
  </div>
</main>
