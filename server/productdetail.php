...
if ($result && mysqli_num_rows($result) > 0) {
  ...
  $product_sub = $row['product_sub'];
} else {

  $product_name = "Product Not Found";
  $product_price = "N/A";
  $product_desc = "The product you're looking for does not exist.";
...
<head>
  ...
  <title><?php echo $product_name; ?> | Panacea</title>
  ...
  <main class="product-main">
    <div class="product-container">
      ...
      <div class="product-info">
        ...
        <p class="subtitle"><?php echo $row['product_sub']; ?></p>
        <p class="price">₱<?php echo number_format($product_price, 2); ?></p>

        <label for="quantity">Quantity</label>
        <input type="number" id="quantity" value="1" min="1" class="border border-gray-300 rounded px-2 py-1">

        <button class="add-btn">Buy Now</button>
        <button class="add-tocart">Add to Cart</button>
        <br>
        <a href="category.php">
            <button class="add-btn">Browse</button>
        </a>

        <div class="description">
          <h2>Product Description</h2>
          <p><?php echo nl2br(htmlspecialchars($product_desc)); ?></p>
        </div>
      </div>
    </div>
  </main>
