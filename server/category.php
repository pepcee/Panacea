...
<head>
  ...
  <title><?php echo $category['category_name']; ?> | Panacea</title>
...
<body class="text-gray-800">
  ...
  <?php while ($product = $products->fetch_assoc()): ?>
    <div class="w-64 border rounded-md p-4 text-center shadow hover:shadow-lg transition-transform duration-300 transform hover:scale-105 cursor-pointer relative bg-white ml-14 mt-6">
      <a href="productdetail.php?product_id=<?php echo $product['product_id']; ?>">
        <img src="../<?php echo $product['product_image']; ?>" alt="<?php echo htmlspecialchars($product['product_name']); ?>" class="mx-auto my-4 h-40 object-contain">
        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">
            <?php echo htmlspecialchars($product['product_tags']); ?>
        </p>
        <h3 class="text-sm font-semibold mb-1"><?php echo htmlspecialchars($product['product_name']); ?></h3>
        <p class="text-base text-customGreen-700 text-center font-bold">₱<?php echo number_format($product['product_price'], 2); ?></p>
      </a>
    </div>
  <?php endwhile; ?>
  </div>
  ...
  <footer class="bg-customBlue text-white pt-10 pb-4 px-6 mt-10">
    <div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
      ...
      <div>
        ...
        <p class="text-sm mb-1">ADDRESS:<br>Gabriela ParkCaster Hills<br>Philippines</p>
        <p class="text-sm mt-2">PHONE: <br>+(63) 99 450 4823</p>
        <p class="text-sm mt-2">EMAIL: <br>panacea@gmai.com.ph</p>
        <p class="text-sm mt-2">WORKING DAYS/HOURS: <br>Mon – Sun / 8:00 AM – 6:00 PM</p>
